...
CMD ["sh", "-c", "cd /app/smart_hrms && python update_admin_roles.py && mkdir -p /tmp/hrms_sessions && gunicorn --bind 0.0.0.0:${PORT:-8000} --workers ${WEB_CONCURRENCY:-2} --worker-class sync --timeout 120 --keep-alive 5 --max-requests 1000 --max-requests-jitter 100 --log-level warning --access-logfile - --error-logfile - run:app"]
